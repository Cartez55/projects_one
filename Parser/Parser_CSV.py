...
with open("export_r.csv", 'r', newline="", encoding="UTF-8") as fp:
    ...
    adress_client = [
        'rvvagner@sberbank.ru;' ' Vishnyakova.Ol.Al1@sberbank.ru;' 'Pinchuk.A.And@sberbank.ru;' 'sysaprykina@sberbank.ru;']
    ...
    for row in reader:
        ...
        if row["Дата"] == date:
            ...
            # mail.Send()
            print(
                'Добрый день!' + '\v' + 'Это автоматическая рассылка!' + "\n\n" + "Изменения в сценариях за " + date + ':' + '\n\n' + \
                row["Номер и название SC/FAQ"] + '\v' + "____" + "\v\v" + row[
                    "Описание изменений"] + '\n\n' + )
